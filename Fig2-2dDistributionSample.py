...
[N,s,U] = [1e9,1e-2,1e-5]
[sim_start,sim_end,snapshot] = [1.313e4,2e4,1.313e4]
...
fit_distr_2d = ax1.pcolormesh(distr_grid.transpose(),cmap=plt.cm.gray_r)
cbar = plt.colorbar(fit_distr_2d)
ax1.axis('tight')        
ax1.set_xticks(np.arange(distr_grid.shape[1])+0.5)
ax1.set_yticks(np.arange(distr_grid.shape[0])+0.5)        
ax1.set_xticklabels(class_xlabels[1:],rotation=90)
ax1.set_yticklabels(class_ylabels[1:])        
ax1.set_xlabel('Beneficial Mutations Trait 1',fontsize=18,labelpad=20)
ax1.set_ylabel('Beneficial Mutations Trait 2',fontsize=18,labelpad=10)
...
fig1.savefig('./figures/2dDistributionSample.pdf')
